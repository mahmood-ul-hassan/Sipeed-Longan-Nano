...
void LCD_ShowChar_tn(u16 x,u16 y,u8 num,u8 mode,u16 color)
{
    u8 temp;
    u8 pos,t;
	u16 x0=x;
	LCD_Address_Set(x,y,x+32-1,y+50-1);      //设置光标位置 

	for(pos=0;pos<50;pos++)
	{ 
		temp=BigFont[(u16)num*200+pos*4];		 //调用1608字体
		for(t=0;t<8;t++)
		{                 
			if(temp&0x80)LCD_WR_DATA(color);
			else LCD_WR_DATA(BACK_COLOR);
			temp<<=1;
			x++;
		}
		temp=BigFont[(u16)num*200+pos*4+1];		 //调用1608字体
		for(t=0;t<8;t++)
		{                 
			if(temp&0x80)LCD_WR_DATA(color);
			else LCD_WR_DATA(BACK_COLOR);
		...
		temp=BigFont[(u16)num*200+pos*4+2];		 //调用1608字体
		for(t=0;t<8;t++)
		{                 
			if(temp&0x80)LCD_WR_DATA(color);
			else LCD_WR_DATA(BACK_COLOR);
		...
		temp=BigFont[(u16)num*200+pos*4+3];		 //调用1608字体
		for(t=0;t<8;t++)
		{                 
			if(temp&0x80)LCD_WR_DATA(color);
			else LCD_WR_DATA(BACK_COLOR);
...
/*
	for(pos=0;pos<16;pos++)
	{ 
		temp=BigFontNo[(u16)num*32+pos*2];		 //调用1608字体
		for(t=0;t<8;t++)
		{                 
			if(temp&0x80)LCD_WR_DATA(color);
			else LCD_WR_DATA(BACK_COLOR);
		...
		temp=BigFontNo[(u16)num*32+pos*2+1];		 //调用1608字体
		for(t=0;t<8;t++)
		{                 
			if(temp&0x80)LCD_WR_DATA(color);
			else LCD_WR_DATA(BACK_COLOR);
...
void LCD_ShowNum_tn(u16 x,u16 y,u16 num,u8 len,u16 color)
{         	
	u8 t,temp;
	u8 enshow=0;
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
//				LCD_ShowChar_tn(x+16*t,y,0,0,color);
				LCD_ShowChar_tn(x+32*t,y,0,0,color);
				continue;
			}else enshow=1; 
		 	 
		}
//	 	LCD_ShowChar_tn(x+16*t,y,temp+1,0,color); 
	 	LCD_ShowChar_tn(x+32*t,y,temp+1,0,color); 
	}
} 
